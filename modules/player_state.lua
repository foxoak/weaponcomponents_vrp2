...
local function tinta(self)
  ...
  vRP.EXT.GUI:registerMenuBuilder("tinta", function(menu)
    menu.title = "Escolha qual arma pintar"
    menu.css.header_color = "rgba(255,125,0,0.75)"
    local user = menu.user
	  local weapons = self.remote.getWeapons(user.source)
	  for k,v in pairs(weapons) do 
  		menu:addOption(get_wname(k), m_wtint_equip, "pintar este arma", k)
	  end
  end)
end
  ...
  local function m_wcomp_equip(menu, weapon)
    ...
    --print(citem.args[2])
      ...
      if self.remote.giveWeaponsComponents(user.source, citem.args[2], weapon) then
        ...
        vRP.EXT.Base.remote._notify(user.source,"Não compatível com esta arma")   
        user:closeMenu(menu)     
      end
  end

  vRP.EXT.GUI:registerMenuBuilder("componente", function(menu)
    menu.title = "Escolha qual arma equipar"
    menu.css.header_color = "rgba(255,125,0,0.75)"
    local user = menu.user
	  local weapons = self.remote.getWeapons(user.source)
	  for k,v in pairs(weapons) do 
...
-- wcomp
  ...
  local function i_wcomp_menu(args, menu)
    menu:addOption(lang.item.wcomp.equip.title(), function(menu)
      menu.user:openMenu("componente", {componente = args[2]})
    end)
  end
...
-- wtint
  ...
  local function i_wtint_menu(args, menu)
      ...
      menu.user:openMenu("tinta", {tintname = args[2]})
    end)
  end
